...
export const makeAssessment = (id,title, minScore: number,maxScore: number, scoring: ScoringInterface[], questions: QuestionInterface[], image='',calcQuestions: (any) => any = defaultCalcQuestion ):AssessmentInterface => {
  return {
    id,
    title,
    minScore,
...
const PostTraumaticStressScoring0 = makeScoring(1,1,1,'LOW',
...
const PostTraumaticStressScoring1 = makeScoring(2,2,2,'MOD',
...
const PostTraumaticStressScoring2 = makeScoring(3,3,3,'HIGH',
...
const DepressionScoring3 = makeScoring(4,4,4,'LOW',
...
const DepressionScoring4 = makeScoring(5,5,5,'MOD',
...
const DepressionScoring5 = makeScoring(6,6,6,'HIGH',
...
const AlcoholDrugsScoring9 = makeScoring(10,49,49,'LOW',
...
const AlcoholDrugsScoring10 = makeScoring(11,50,50,'MOD',
...
const AlcoholDrugsScoring11 = makeScoring(12,51,51,'HIGH',
...
const AnxietyScoring16 = makeScoring(17,56,56,'LOW',
...
const AnxietyScoring17 = makeScoring(18,57,57,'MOD',
...
const AnxietyScoring18 = makeScoring(19,58,58,'HIGH',
...
const PanicScoring22 = makeScoring(23,74,74,'LOW',
...
const PanicScoring23 = makeScoring(24,75,75,'MOD',
...
const PanicScoring24 = makeScoring(25,76,76,'HIGH',
...
const PhysicalInjuryResilienceScoring25 = makeScoring(26,77,77,'LOW',
...
const PhysicalInjuryResilienceScoring26 = makeScoring(27,78,78,'MOD',
...
const PhysicalInjuryResilienceScoring27 = makeScoring(28,79,79,'HIGH',
...
const calcAlcDrugQuestions = function(values: any){
      ...
      let conditionalQuestionsLabels = this.questions.filter(q => {
         if(chosen.length > 0 && parseInt(q.id) > 6){
           return true;
         }
         return false;
      });
      ...
      console.log(chosen);
      if(chosen.length > 0){
        conditionalQuestionsLabels.map((ql) => {
          conditionalQuestions.push(ql);
          chosen.map((qcId) => {
            var subQuestion = questionsMap[qcId];
            if(subQuestion){
              conditionalQuestions.push(
                makeQuestion(ql.id + '_' + qcId, subQuestion.title, 'select', ql.choices)
              )
            }
          });
        })
      ...
      return substanceQuestions.concat(conditionalQuestions);
}
...
const assessmentsRaw: AssessmentInterface[] = [
  makeAssessment(1,'Alcohol and Drugs', 0, 38, AlcoholDrugsList,alcDrugsQuestions,friendsImage,calcAlcDrugQuestions),
  //makeAssessment(2,'Post-Traumatic Stress', 17, 85,PostTraumaticStressList,maritalSatisfactionQuestions,marriageImage),
